...
import { RecipeSchema } from '@meal-planner/shared';
  ...
  const handleSearchRecipes = async () => {
    ...
    try {
      ...
      const validRecipes = data.filter((recipe: any) => {
        ...
        if (!result.success) {
          console.error('Invalid recipe data from by-ingredients endpoint:', recipe, result.error.format());
          return false;
        }
        return true;
      });
    ...
    >
      ...
      <Stack spacing={3} sx={{ mb: 4, maxWidth: 600, mx: 'auto' }}>
        <Box>
          ...
          />
          <Button
            startDecorator={loading ? <Loader2 className="spin" size={18} /> : <Search />}
            size="lg"
            color="primary"
            variant="solid"
      ...
      {loading ? (
        ...
        <Box sx={{ textAlign: 'center', color: 'text.secondary', py: 8 }}>
          ...
          />
          <Typography level="body-lg">No recipes found with these ingredients.</Typography>
          <Typography level="body-sm">Try broadening your search or adding more ingredients.</Typography>
        </Box>
      ) : (
        <Stack
          direction="row"
