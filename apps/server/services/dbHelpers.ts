  ...
  return result[0].values.map((row: unknown[]) => {
    ...
    columns.forEach((col: string, idx: number) => {
      if (col === 'ingredienti' || col === 'stagioni') {
        obj[col as keyof Recipe] = row[idx] ? JSON.parse(row[idx] as string) : [];
      } else if (col === 'link') {
        obj[col as keyof Recipe] = typeof row[idx] === 'string' ? row[idx] as any : (row[idx] == null ? undefined : String(row[idx]) as any);
      } else {
...
export function parseConfig(result: QueryExecResult[]): Config {
  ...
  if (result.length > 0 && result[0].values.length > 0) {
    ...
    config.menuOptions = typeof menuOptionsValue === 'string' ? JSON.parse(menuOptionsValue) as MenuOptions : {} as MenuOptions;
  }
  return config;
}
