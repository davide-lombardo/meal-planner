export function generateMenu(recipes, history = [], config) {
    ...
    const recentlyUsed = new Set(history.slice(-maxWeeks).flatMap((menu) => [...menu.pranzo, ...menu.cena].filter(r => r && r.id).map(r => r.id)));
    const canUseRecipe = (recipe, mealType) => {
        ...
        if (recipe.tipo && recipe.tipo !== mealType)
            return false;
        if (useQuotas && weeklyQuotas && weeklyQuotas[recipe.categoria || ''] <= 0)
            return false;
        return true;
    };
    ...
    const selectMeal = (mealType, dayIndex) => {
        if (mealType === 'cena') {
            if (dayIndex === 5)
                return { id: 'pizza', nome: 'Pizza', tipo: 'cena', ingredienti: [] };
            if (dayIndex === 6)
                return { id: 'libero', nome: 'Libero', tipo: 'cena', ingredienti: [] };
        }
        let available = getAvailableRecipes(mealType);
        if (available.length === 0)
            available = getAvailableRecipes(mealType, true);
        if (available.length === 0)
            available = recipes.filter(r => (!r.tipo || r.tipo === mealType) && !recentlyUsed.has(r.id));
        if (available.length === 0)
...
export function generateHtmlEmail(menu, recipes) {
    const daysOfWeek = ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato', 'Domenica'];
